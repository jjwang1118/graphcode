...
| `inherits` | 邊 | 類別繼承，`class → class`。**只連專案內**——base 是 builtin 或外部套件時不產生邊，只計數，因為 `ext:` 節點不是 class |
...
**resolve 不是 facts 的下一站，是它呼叫的一層** — 管線圖上那組雙向箭頭是「送一個名字去查 → 拿回接好的邊」，不是管線倒流。resolve 仍然獨立測試、獨立逐語言擴充。
