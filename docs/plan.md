...
| 階段 | 內容 |
|---|---|
| 階段 3 | zip 上傳，連同 zip-slip / zip bomb 防護 |
| 階段 4 | 大圖可讀性：上千節點的收合、依目錄分群、邊聚合成帶權重的邊。**1.11 只有 87 個節點就已經覺得不好看**，所以這階段不只是「大圖」問題，layout 與樣式本身也要重做 |
| 之後 | `class` / `function` 層與 `calls` / `inherits` 邊；多語言（TS）；語意縮放 |
