...
| 5.4 | `inherits` 邊（**B**） | `class Foo(Bar)` 連到 `Bar` 的節點 | docs/backend/resolve.md | 待辦 | | 本專案只有 11 條，`Bar` 是寫在原始碼裡的名字，靠 5.5 的宣告表就解得掉。風險中等 |
| 5.5 | **作用域感知的宣告表** | `foo()` 這一類的名字能查到唯一的宣告 | docs/backend/resolve.md | 待辦 | | 5.6 的前置。跟現有架構同一個模式——`imports` 靠 `resolve/index.py` 的「路徑 → 檔案」索引，`calls` 靠「名字 → 宣告」索引，**兩者都由 resolve 建、parse 不需要知道**。表是 5.1 的產物，不必另外維護。**關鍵是查表順序不是表本身**：① 本檔宣告 ② 本檔 import 進來 ③ 全域——就是 Python 自己的名字解析規則。全域有 10 個同名函式（`parse`×3、`target`×3…），扁平 dict 會亂連 |
