...
| # | 項目 | 完成條件 | 對應文件 | 狀態 | 最後修改 | 備註 |
|---|---|---|---|---|---|---|
| 5.1 | `class` / `function` 節點 ＋ `defines` 邊（**A**） | `def foo():` 產生節點、`foo()` 不產生；圖能回答「這個檔案裡有什麼」 | docs/backend/parse.md ＋ graph.md §2.5/§3.8 ＋ graph_schema.md | **完成（圖例待補）** | 2026-09-08 | **唯一不會解錯的一塊**——宣告直接變節點，不經 resolve（CLAUDE.md parse 那節寫的）。id 規則現成：`class:src/app.py::Runner`、`function:src/app.py::Runner.run`。前端**結構**不用改：`type` 是資料不是結構，`transform.ts` 原封轉發，`nodes` 陣列只是多幾種 `type`。但 `style.ts` 沒有 `class` / `function` 的樣式，那些節點會退回預設而跟 `file` 同色同形——**要看得出誰是誰就得補兩個顏色**（2026-09-07 讀 code 更正，原本寫的「完全不用改」是錯的）。<br>**做完的結果**：本專案 72 → 323 節點（class 25、function 225）、250 條 `defines`。`Defines` fact 多一個 `overload` 欄位，去重規則見 graph.md D4——不去重的話分析 pydantic 會因為重複 id 直接丟 `BuildError`。配色：class 粉五邊形、function 暖灰三角形，`defines` 邊灰紫。<br>**還缺一塊（2026-09-08 發現）**：`Sidebar.tsx` 的圖例只列四種節點兩種邊，畫布上的 class／function／`defines` 在圖例裡查不到。圖例是人看的窮舉清單，**不受「`type` 是資料不是結構」保護**——規格已寫進 Frontend.md ›「圖例要列出所有畫得出來的型別」。連帶 `frontend/src/api/types.ts:64` 的 `level` 註解還寫著「`contains` 樹」，也過期了 |
