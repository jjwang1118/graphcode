...
| 4.3 | 展開聚合的細節 | 聚合掉的東西在畫面上看得到 | docs/Frontend.md › 細節框 | 邊完成、節點待辦 | 2026-09-07 | **原本寫「資料都已經在了，只差顯示的地方」——那句是錯的**，只在檔案層成立：收合是後端做的，`views.py` 的 `_edges()` 重建邊時只留 `weight`，`module` / `name` / `line` 全丟了，而前端預設就在第 3 層，所以 `data.detail` 一直是空字串。**做法**：後端收合時把被合併掉的原始 `imports` 邊留在 `properties.sources`（規則 C11，`contains` 不揹），前端 `transform.ts` 把它翻成 `ImportDetail[]` 並依「來源檔 → 目標檔」分組——**認識後端形狀的只有 `transform.ts` 一處**，4.4 改成打 API 查全量圖時只換那個函式。顯示是 `DetailBox.tsx`，**兩個畫布共用**：點邊釘住、框外按下就關（關閉寫在框自己身上，所以畫布只需要「開」一個 handler）。3D 另加 `linkHoverPrecision(6)`，線太細否則點不到。**實測**：那條 `weight=16` 的粗線背後只有 4 個檔案對，全 63 條收合邊的檔案對最多 8——框最多 8 行，看得完；含 `sources` 的 JSON 46 KB，仍低於檔案層的 64 KB。行號**放在標題列**（同一個 `from ... import a, b, c` 的名字共用一行，跟在每個名字後面是重複；分組後只出現一次）。框有 120ms 淡入動畫與亮邊。**剩下節點的細節**：`internal_imports` 只是一個數字、`packages` 有名單但沒地方顯示 |
...
### 階段 5 — Python 內部結構（圖模型第二階段）

未開工。把圖從「檔案彼此 import」推進到「**檔案裡有什麼、它們怎麼互相呼叫**」，也就是 CLAUDE.md 圖模型那節「schema 已定義、尚未填充」的 `class` / `function` 節點與 `defines` / `calls` / `inherits` 邊。

**規模**（本專案實測，46 個 `.py`）：`class` 23、`function` 213（含 12 個方法）、呼叫點 **731**、繼承 11。節點會從 112 變成約 **348（3.1 倍）**。

切成三塊，難度差一個量級：**A 不會做錯、B 好解、C 有準確度風險**。

| # | 項目 | 完成條件 | 對應文件 | 狀態 | 最後修改 | 備註 |
|---|---|---|---|---|---|---|
| 5.1 | `class` / `function` 節點 ＋ `defines` 邊（**A**） | `def foo():` 產生節點、`foo()` 不產生；圖能回答「這個檔案裡有什麼」 | docs/backend/parse.md ＋ facts.md §2.3/§3.2 ＋ graph_schema.md | **完成** | 2026-09-10 | **唯一不會解錯的一塊**——宣告直接變節點，不經 resolve（CLAUDE.md parse 那節寫的）。id 規則現成：`class:src/app.py::Runner`、`function:src/app.py::Runner.run`。前端**結構**不用改：`type` 是資料不是結構，`transform.ts` 原封轉發，`nodes` 陣列只是多幾種 `type`。但 `style.ts` 沒有 `class` / `function` 的樣式，那些節點會退回預設而跟 `file` 同色同形——**要看得出誰是誰就得補兩個顏色**（2026-09-07 讀 code 更正，原本寫的「完全不用改」是錯的）。<br>**做完的結果**：本專案 72 → 323 節點（class 25、function 225）、250 條 `defines`。`Defines` fact 多一個 `overload` 欄位，去重規則見 facts.md D4——不去重的話分析 pydantic 會因為重複 id 直接丟 `BuildError`。配色：class 粉五邊形、function 暖灰三角形，`defines` 邊灰紫。<br>**圖例已補（2026-09-10）**：`Sidebar.tsx` 加上 class（粉五邊形）、function（暖灰三角形）與 `defines`（灰紫線），`Legend` 的 `shape` 多兩個 `clipPath`。圖例是人看的窮舉清單，**不受「`type` 是資料不是結構」保護**——規格見 Frontend.md ›「圖例要列出所有畫得出來的型別」。同一次順手修掉 4.1 漏掉的地方：圖例裡 `imports` 畫成 3.5 而 `contains` 是 2，跟「三種邊同寬」的規格相反，已統一為 2。**仍未處理**：`frontend/src/api/types.ts:64` 的 `level` 註解還寫著「`contains` 樹」，5.3 之後已是 `contains` ＋ `defines` |
| 5.2 | `Fact → 節點/邊` 的對應方式 | 加一種 Fact 不必在 build 加一個 `if` | docs/backend/facts.md（新開）＋ parse.md | **完成** | 2026-09-17 | **open-decisions 第 3 項結案。** 真正的 `if` 不在 `build.py` 而在 `pipeline.py` 的兩段分流；5.4（`Inherits`）與 5.6（`Calls`）會讓它變四段。<br>**做法**：新開一層 `app/facts/`，裡面是一張**型別 → 產生器**的表（`PRODUCERS`）與套用它的迴圈（`to_graph`），`pipeline.py` 變成不認識具體型別的三行。`app/graph/declarations.py` 搬進來，演算法一行未改。<br>**CLAUDE.md 原本寫的「讓 Fact 自己宣告它產生什麼邊」做不到**，實作前先講、文件已改寫：`Import("fastapi")` 在 resolve 之前不知道 target 是 `ext:` 還是某個 `file:`；讓 Fact 宣告節點則要 `import app.models`，破掉 parse.md §2.2／§8.5 的界線。理由寫進 `facts.md` §8.6 與 `parse.md` §8.9。<br>**產出零變化**：同一棵樹餵給新舊兩份程式碼，`document()` 的 JSON 8158 行完全相同（`git worktree` 出一份 HEAD 對跑）。測試 166 → 171，既有斷言一條未改
| 5.3 | 收合層級吃 `defines` | class / function 在收合時有正確的深度 | docs/backend/graph.md › 收合 | **完成** | 2026-09-07 | `views.py` 的 `_targets()` 目前只認 `contains`，掛在 `defines` 上的節點會被當成深度 0 的孤兒。CLAUDE.md 圖模型那節已經寫明代價：「沿層級展開時**兩種邊都要吃**」。**必然要動今天剛加 C11 的那個檔案**。<br>**不能等到 5.1 之後才做**：前端預設就是 `level=3`，5.1 單獨上線會讓預設畫面從 71 個節點變成 307 個——那 236 個宣告節點在每一層都收不掉。<br>**做完的結果**：`_TREE_EDGES = {contains, defines}`，函式比它所在的檔案深一層。**但預設值要換**，見下面的驗收段 |
| 5.4 | `inherits` 邊（**B**） | `class Foo(Bar)` 連到 `Bar` 的節點 | docs/backend/resolve.md ＋ parse.md §3.4 ＋ facts.md §3.4 | **完成** | 2026-09-17 | **「靠 5.5 的宣告表」那句是對的，所以 5.4 先做了那張表的檔案層級版**（`app/resolve/names.py`）：本檔宣告 ＋ 本檔 import 進來的名字，查表順序就是 Python 自己的規則，**沒有全域搜尋**（跨檔案找同名的會連出一堆假邊）。5.5 要加的是作用域感知，表的形狀不必改。<br>**`Resolver` protocol 一行沒改**——「`Bar` 從哪個檔案來」就是解本檔那一筆已存在的 `Import`，直接重用 `resolver.target()`，相對 import／`__init__.py`／先窄後寬全部沿用。`pipeline.py` 只多一行建表，**分流那部分一行未改**（5.2 換來的）。<br>**只連專案內的 class**：base 是 builtin 或外部套件就不產生邊，計進新的 `meta.unresolved_inherits`（前後端契約多一欄）。不接到 `ext:` 是為了讓 `inherits` 嚴格維持 class → class。<br>**原本想把解不到的 base 記在子類別節點的 `properties`，做不到**：節點是 `DeclarationProducer` 產的，兩個產生器產同一個 id 會撞 `BuildError`，所以那筆資訊只剩計數，呈現留給 5.7。<br>**實測**：本專案 **0 條邊、13 筆全落在 builtins／外部套件**（13 條繼承沒有一條指向專案內，所以本專案驗收不了這一項）；starlette **25 條**、networkx **152 條（跨檔案 29）**，與動手前用拋棄式腳本普查的數字一致。<br>**已知限制**：巢狀類別以裸名被繼承（`Outer` 裡的 `Inner(AntiAtlasView)`）查不到，networkx 實測 2 筆——那要作用域感知，是 5.5。<br>測試 171 → 196（parse 8、names 10、inherits 7），ruff 乾淨，mypy 只剩 baseline 本來就有的 3 個 |
| 5.5 | **作用域感知的宣告表** | `foo()` 這一類的名字能查到唯一的宣告 | docs/backend/resolve.md | 待辦（**檔案層級那一半已完成**） | 2026-09-17 | **表已經存在**：5.4 建了 `app/resolve/names.py`，查表順序（本檔宣告 → 本檔 import → 查不到）也已經是這裡寫的那一套，只差**函式內的作用域與遮蔽**。5.6 的前置。跟現有架構同一個模式——`imports` 靠 `resolve/index.py` 的「路徑 → 檔案」索引，`calls` 靠「名字 → 宣告」索引，**兩者都由 resolve 建、parse 不需要知道**。表是 5.1 的產物，不必另外維護。**關鍵是查表順序不是表本身**：① 本檔宣告 ② 本檔 import 進來 ③ 全域——就是 Python 自己的名字解析規則。全域有 10 個同名函式（`parse`×3、`target`×3…），扁平 dict 會亂連 |
| 5.6 | `calls` 邊（**C**） | 「改這個函式會波及誰」答得出來 | docs/backend/resolve.md | 待辦 | | **準確度風險最高**。實測 731 個呼叫點：`foo()` 504 筆用 5.5 的表 **100% 解得掉、零歧義**（本檔宣告 140、本檔 import 125、內建／外部 239）；`模組.foo()` 27 與 `self.foo()` 4 再走一層表也解得掉；**`變數.foo()` 110 與鏈式 `a.b.foo()` 86 解不掉**——`ast` 沒有型別資訊，那是型別推論的領域。合計 **67% 有明確答案、27% 無解**。<br>**2026-09-07 補測：型別註解能砍掉一半的無解。** 133 筆 `變數.foo()`（排除 `self`/`cls`，鏈式另計；與上面 110 那個數字口徑不同，不能直接比）之中，靠參數註解與變數註解查得到型別的有 70 筆：45 筆是內建／標準庫（`list[str]`、`Path`）＝**可判定為外部呼叫**、2 筆外部套件、16 筆指向專案內的 class，其中 **14 筆真的接得到方法節點、零誤判**。無解從 133 降到 70。<br>**但這條線索不該當主力**：本專案型別註解 100% 是因為跑 mypy，沒跑 mypy 的專案會直接消失。<br>**而且它不是節點的 `properties`**——型別對照表是「呼叫點所在函式」的區域資訊，屬於 `Calls` fact 的欄位（如 `receiver_type`），在 5.1 的節點上存 `params`/`returns` 幫不上忙 |
| 5.7 | 解不掉的怎麼呈現 | 圖不會假裝完整 | docs/backend/resolve.md | 待辦 | | **5.6 開工前要拍板**。選項：丟掉（不完整且看不出哪裡缺，答案會少算）／標 `properties.unresolved` 畫虛線（誠實但多噪音）／只做同檔案內（不會錯但答不了跨檔案）／換型別推論（推翻 `parse.md` 選 ast 的決定）。**現有先例**：`meta.unresolved_imports` 與 `properties.ambiguous`，照同一套處置最一致。**5.4 又多了一個**：`meta.unresolved_inherits`（13 筆，全是 builtins 與外部套件）——它證明了「一個數字」不夠用，因為那 13 筆不是錯誤，跟 `unresolved_imports` 的「接不到目標」混在一起看會誤判分析品質。<br>**2026-09-07：前提變了。** 5.6 的補測顯示無解那一塊有近半可以靠型別註解變成「確定是外部呼叫」或「明確的邊」，所以這裡要決定的其實是**三分類**（本專案內／確定外部／真的不知道），不是二分類 |

**兩個跨項的提醒**：

- **「呼叫」的定義本身有邊界。** `ast.Call` 抓得到 `foo()`，抓不到 `map(foo, xs)` 那種「把函式當值傳出去」——那也是依賴。要不要算，是設計問題不是實作問題。
- **沒有 ground truth。** 「解析準確度七成」目前只是分類統計，不是驗證。要有一組人工核對過的樣本，否則說不出做對了沒有。

**驗收基準已經變了**（2026-09-07 實測，本專案）：

| `level` | 節點 | 內容 |
|---|---|---|
| 2 | 44 | 只有目錄與淺層檔案——**新的甜蜜點** |
| 3（目前的預設） | 228 | 深層檔案的函式收回檔案了，但 `tests/` 那種淺層檔案的函式還在 |
| 4 | 311 | 幾乎全展開 |
| 不收合 | 323 | 全部 |

深度是深度——淺目錄裡的東西本來就比較早見底，只有檔案節點時就已經如此（`README.md` 在第 2 層保留自己）。**`App.tsx` 的預設 `level` 要不要從 3 改成 2 還沒改**，等目視確認。
...
| 項目 | 內容 |
|---|---|
| 大 repo 基準 | 找一個上千節點的真實專案跑通，記下節點／邊數與各層級的數字，當作可讀性的驗收基準。**2026-09-06 原列為 4.2，裁決移到這裡**。<br>**2026-09-07 撞到一個既有的擋路石**：`analyze(pydantic)` 跑不完，卡在 `build._import_cycles` 的 `nx.simple_cycles`——它列舉**所有**簡單環，而 pydantic 的 import 圖有天文數字級的環（2 萬個只花 0 秒，還遠遠沒列完）。這條路徑只吃 `imports` 邊，**與 5.1 無關**，是階段 1 就埋著的。要跑大 repo 就得先處理：設上限、改成只回強連通分量、或加逾時 |
