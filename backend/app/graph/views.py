...
為什麼需要收合：不收合時有 406 個節點、153 條聚合後的 imports 邊，而且
`app/models/__init__.py` 一個節點就吃掉 22 條入邊，畫面上是蜘蛛網中心。收到第
2 層之後剩 42 個節點、21 條邊，最忙的只剩 2 條入邊——因為「所有檔案都 import
models」在目錄層本來就該是**一條**邊。
