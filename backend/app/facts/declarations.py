...
不查任何索引，是一個純粹的對應層，餵一份假的 fact 就測得動；`Context` 收下了
也用不到。

**沒有計數**：宣告不需要解析，抽得到就是對的。檔案整份解析失敗的情況由
`meta.parse_failures` 涵蓋。規則見 docs/backend/facts.md。
"""
...
class DeclarationProducer:
    def produce(
        self, facts: Mapping[str, Sequence[Defines]], context: Context
    ) -> Production:
        return to_nodes(facts)


def to_nodes(facts: Mapping[str, Sequence[Defines]]) -> Production:
    ...
    return Production(nodes=tuple(nodes), edges=tuple(edges))
